...
- clipboard와 richer notification payload를 평문 해석 없이 그대로 중계하는 blind relay 구조
...
- Android 알림 텍스트나 이미지 자산도 평문으로 해석하면 안 돼요.
...
- notification payload가 커져도 서버는 `content_type`, `nonce`, `header_aad`, `ciphertext` 같은 envelope 외곽 메타데이터만 검증해야 해요.
